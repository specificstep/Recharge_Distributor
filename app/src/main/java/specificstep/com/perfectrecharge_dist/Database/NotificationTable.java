package specificstep.com.perfectrecharge_dist.Database;
...
//        values.put(KEY_DELETE_FLAG, model.deleteFlag);
        int updateId = db.update(TABLE_NAME, values, whereClause, null);
        Log.d("Database", "Update Notification data : " + updateId);
        db.close();
    }
...
//                notificationModel.deleteFlag = cursor.getString(cursor.getColumnIndex(KEY_DELETE_FLAG));
    ...
    public int getNumberOfNotificationRecord() {
        ...
        // SELECT * FROM tbl_notify ORDER BY id DESC LIMIT 1
        SQLiteDatabase db = openHelper.getWritableDatabase();
        Cursor cursor = db.query(TABLE_NAME, new String[]{KEY_NOTIFY_ID}, KEY_READ_FLAG + "=?", new String[]{"0"}, null, null, null);
